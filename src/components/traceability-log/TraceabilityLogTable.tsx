            ...
            {data.map((log, index) => (
              ...
              >
                ...
                <td className="whitespace-nowrap px-4 py-3 text-center">
                  <span
                    className={`inline-flex items-center gap-1 whitespace-nowrap rounded-full border px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${log.isFinish
                  ...
                  >
                    <span
                      className={`size-1.5 rounded-full ${log.isFinish ? "bg-emerald-500" : "bg-amber-500"
                        }`}
                    />
...
function TraceabilityLogMobileCard({
  ...
  return (
    <article
      className={`rounded-lg border p-3.5 shadow-xs ${log.status
...
function ProgressBadge({ isFinish }: { isFinish: boolean }) {
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${isFinish
          ? "border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-500/15 dark:text-emerald-400"
          : "border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/15 dark:text-amber-400"
        }`}
    >
      <span
        className={`size-1.5 rounded-full ${isFinish ? "bg-emerald-500" : "bg-amber-500"
          }`}
      />
...
function StatusBadge({ status }: { status: boolean }) {
  return (
    <span
      className={`inline-flex rounded-full border px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${status
